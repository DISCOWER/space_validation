__author__ = "Joris Verhagen & Elias Krantz"
__contact__ = "jorisv@kth.se & eliaskra@kth.se"
...
class MPCNode(Node):
    def __init__(self):
        ...
        # which experiment do we run? (from which folder do we load the trajectories?)
        exp = self.declare_parameter('experiment', 'exp_1').value

        # what kind of controller do we use? 
        # 'offset_free' should be true in accordance to the paper
        # 'feedback_equivalence' should be true for underwater experiments (but can still be turned off if desired)
        self.offset_free = self.declare_parameter('offset_free', True).value
        self.feedback_equivalence = self.declare_parameter('feedback_equivalence', True).value

        # Offsets for the reference trajectory, to compensate for the 
        # different scale of the space platform and the pool
        ...
        # and use that dynamics model to create the MPC solver
        # we always create MPC and MPCFBL, even if we don't use feedback_equivalence according to the 
        # setting below, to be consistent in later analysis
        ...
        self.reference_p_v_pub = self.create_publisher(
            VehicleLocalPosition,
            'space_validation/reference_pos_vel',
            10)
        self.reference_q_pub = self.create_publisher(
            VehicleAttitude,
            'space_validation/reference_q',
            10)
        self.reference_w_pub = self.create_publisher(
            VehicleAngularVelocity,
            'space_validation/reference_w',
            10)
        ...
        # EKF variables (other is the model that is not being controlled)
        # We run an EKF for both the controlled model and the other model,
        # one is for the offset-free MPC, the other is later analyzing the results
        ...
        path = os.path.abspath(os.path.join(os.path.expanduser("~"), f'space_validation_ws/src/space_validation/space_validation/Planning/solutions/{exp}/'))
